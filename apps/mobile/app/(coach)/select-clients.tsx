/**
 * select-clients.tsx (coach) — Full-screen client selector, scoped to the
 * clients explicitly assigned to this coach (coach_client_assignments),
 * unlike the admin version which lists every client in the tenant.
 *
 * Flow:
 *   coach-appointments.tsx → router.push('/(coach)/select-clients')
 *   user searches, selects, taps Confirmar
 *   router.back() → coach-appointments.tsx reads from clientSelectionStore
 */
...
import { getCoachClients, CLIENT_LEVEL_LABELS, type ClientWithPlan } from '@/services/profiles.service';

export default function CoachSelectClientsScreen() {
  ...
  const handleConfirm = () => router.back();
          ...
          ListEmptyComponent={
            <View style={s.center}>
              ...
              <Text style={{ color: T.textMuted, fontSize: 14, textAlign: 'center' }}>
                {query
                  ? t('admin.selectClients.noResultsFor', { query })
                  : t('coach.selectClients.noClientsAssigned')}
              </Text>
            </View>
          }
          renderItem={({ item }) => {
            ...
            return (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {levelLabel ? (
                    <Text style={{ fontSize: 11, color: T.textMuted, marginTop: 1 }}>{levelLabel}</Text>
                  ) : null}
                </View>
                <View style={[s.check, {
