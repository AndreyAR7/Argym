...
export default async function CoachVideosPage({
  ...
  // Same tenant-wide visibility + full CRUD as admin — a coach with
  // content.manage can see and manage every video in the tenant's
  // library (draft/archived included), not only published ones.
  let query = supabase
    .from('videos')
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        <VideoLevelSelect currentLevel={levelFilter} currentStatus={statusFilter} basePath="/coach/videos" />
      </div>
      ...
      {!videos || videos.length === 0 ? (
        <div className="mt-16 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-xs text-[var(--color-muted-foreground)]">
            {statusFilter !== 'all' ? 'No hay videos con este filtro.' : 'Sube el primer video con el botón "Nuevo video".'}
          </p>
        </div>
      ) : (
