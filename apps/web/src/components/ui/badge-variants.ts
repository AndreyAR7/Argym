// Pure data, no JSX — split out of badge.tsx so it can be unit-tested
// without Vitest needing to parse/transform the component's JSX.
export type Variant =
  | 'approved' | 'pending' | 'rejected' | 'blocked'
  | 'active' | 'inactive' | 'suspended' | 'cancelled' | 'expired'
  | 'paid' | 'overdue'
  | 'beginner' | 'intermediate' | 'advanced'
  | 'scheduled' | 'confirmed' | 'completed' | 'no_show' | 'pending_confirmation' | 'postpone_requested'
...
export const styles: Record<Variant, string> = {
  ...
  postpone_requested:   'bg-orange-50   text-orange-700   border-orange-200',
  // Content
