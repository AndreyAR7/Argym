-- ============================================================
-- Migration 000051: fix list_appointments
--
-- Root cause: the WHERE clause filtered by v_role, but v_role
-- can be NULL if the user_roles lookup returns nothing, causing
-- 0 rows silently even for admins.
--
-- Fix: tenant isolation is sufficient. The calling page (/admin)
-- is already protected by middleware that verifies the admin role.
-- Remove the per-row role check from the WHERE clause.
-- ============================================================
...
BEGIN
  ...
  WHERE a.tenant_id = v_tenant_id          -- tenant isolation only
