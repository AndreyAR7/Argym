-- Extends list_appointments (last touched in 000110) for group classes:
-- 1. Exposes max_participants/class_template_id so the calendar can show
--    "8/12" and grey out full classes.
-- 2. Adds each participant's own status to the aggregated JSON (needed for
--    the roster UI to tell pending/confirmed/attended/no_show apart).
-- 3. Fixes visibility: a client who booked a spot in a class matched
--    neither coach_id nor client_id (class instances have client_id=NULL),
--    so they could never see their own class bookings. Adds an
--    appointment_participants membership check alongside the existing
--    coach/client ownership check.
