-- Two real authorization gaps found while verifying 000174 in production
-- against the Ímpetu tenant, whose only admin-tier user now holds the
-- 'full_access' role (not 'admin') after this session's role restructuring:
--
-- 1. analytics_executive_kpis / analytics_revenue_summary /
--    analytics_branch_performance hardcode `r.name IN ('admin','superadmin')`
--    — missed by migration 000169's full_access audit. Result: the entire
--    Analytics dashboard silently returns zero rows for a full_access user.
--
-- 2. The other 10 analytics_* RPCs (monthly_revenue, top_plans,
--    top_promotions, top_users, top_videos, weekly_activity, and all 4
--    detailed_* tables) are SECURITY DEFINER with NO caller check at all —
--    any authenticated user can call them with an arbitrary p_tenant_id and
--    read another tenant's revenue/client/transaction data. A real
--    cross-tenant data leak, unrelated to full_access.
--
-- Fix: one helper (single source of truth for "is this caller an admin-tier
-- user of this tenant") used by every analytics_* function, so this class of
-- gap can't recur one function at a time again.

CREATE OR REPLACE FUNCTION public.analytics_caller_is_admin(p_tenant_id UUID)
RETURNS BOOLEAN
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    ...
    WHERE p.id = auth.uid() AND p.tenant_id = p_tenant_id
      AND r.name IN ('admin','superadmin','full_access')
  );
$$;

REVOKE ALL ON FUNCTION public.analytics_caller_is_admin(UUID) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.analytics_caller_is_admin(UUID) FROM anon;
GRANT EXECUTE ON FUNCTION public.analytics_caller_is_admin(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.analytics_caller_is_admin(UUID) TO service_role;

-- ─────────────────────────────────────────────────────────────
-- 1. Functions that already gated on role — swap in the helper
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  IF NOT public.analytics_caller_is_admin(p_tenant_id) THEN RETURN; END IF;
...
-- ─────────────────────────────────────────────────────────────
-- 2. Functions with NO caller check at all — add one via the helper.
--    Same shape as before, only the WHERE clause changes.
-- ─────────────────────────────────────────────────────────────
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE us.tenant_id = p_tenant_id
    AND us.status IN ('active','expired')
    AND us.start_date BETWEEN p_from AND p_to
    AND public.analytics_caller_is_admin(p_tenant_id)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE public.analytics_caller_is_admin(p_tenant_id)
  GROUP BY w.wk
  ORDER BY w.wk;
$$;
...
-- ─────────────────────────────────────────────────────────────
-- 3. Also fix the brand-new 000174 function to use the same helper
--    instead of a 4th copy of the incomplete 'admin','superadmin' check.
-- ─────────────────────────────────────────────────────────────
